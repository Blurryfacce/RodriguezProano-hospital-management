...
Los 4 doctores precargados en `data.sql` tienen al menos una cita asociada (`fk_citas_doctor`). `DoctorService.eliminar()` no verifica esto antes de borrar. En este test de integración (MockMvc + H2, con la clase de prueba anotada `@Transactional`), el `DELETE` en sí responde 200 (el borrado se difiere hasta el siguiente flush de Hibernate dentro de la misma transacción de test), y la siguiente consulta falla con una violación de integridad referencial expuesta como HTTP 500 con stack trace.

**Actualización (Paso 5, verificado contra el backend real):** el comportamiento "diferido" descrito arriba es un **artefacto del `@Transactional` de la clase de test**, que envuelve todo el método de prueba en una única transacción sin confirmar. Contra el backend real corriendo (una transacción por request, como en producción), `curl -X DELETE /api/doctores/1` devuelve **HTTP 500 inmediatamente en la misma llamada** (la violación de FK ocurre al hacer commit de la transacción de ese request), y el doctor **NO se elimina** (se confirmó con un GET posterior). Es decir: en producción el error es inmediato y el dato queda íntegro; en el test de integración parecía "diferido" solo por cómo se estructuró la prueba. Ambos comportamientos son coherentes entre sí (mismo bug de fondo: falta de verificación de citas antes de eliminar), pero el detalle de *cuándo* surge el error difiere según el contexto transaccional. Ver `informes/hallazgos/06-paso5-e2e-doctores.md` para el hallazgo completo, incluyendo su interacción con el bug de `apiFetch` en el frontend (que termina mostrando un mensaje de **éxito falso** al usuario pese al 500).
